-- YDB6.2: snapshot — готовые вложения карточек (только status = ready), без storage_path и токенов.
-- Видимость как у карточек доски: вызывающий уже прошёл board.view (SECURITY DEFINER).
...
BEGIN
  ...
  SELECT COALESCE(
    jsonb_agg(
      ...
      ORDER BY ca.card_id, ca.uploaded_at ASC, ca.id ASC
    ),
    '[]'::jsonb
  )
...
COMMENT ON FUNCTION public.get_board_snapshot(uuid) IS
  'Aggregated board snapshot for /boards/[boardId]. Includes current_user_id, yandex_disk_integration (no tokens), and card_ready_attachments (ready only, no storage_path).';
