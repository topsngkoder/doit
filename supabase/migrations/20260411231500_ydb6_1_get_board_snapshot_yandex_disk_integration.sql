-- YDB6.1: snapshot — срез интеграции Яндекс.Диска без токенов (спец. / план YDB6.1).
-- status виден всем с board.view; прочие поля — только владельцу доски или sysadmin.
...
COMMENT ON FUNCTION public.get_board_snapshot(uuid) IS
  'Aggregated board snapshot for /boards/[boardId]. Includes current_user_id and yandex_disk_integration (no tokens; owner/sysadmin-only details).';
