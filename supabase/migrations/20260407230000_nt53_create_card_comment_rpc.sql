-- NT5.3: Создание комментария через RPC с доставкой card_comment_new (spec §4.3).
...
REVOKE ALL ON FUNCTION public.create_card_comment(uuid, text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_card_comment(uuid, text, uuid) TO authenticated;

COMMENT ON FUNCTION public.create_card_comment(uuid, text, uuid) IS
'Вставка card_comments с теми же предпосылками, что RLS insert (comments.create); уведомления всем assignees, кроме автора (через enqueue_notification_event).';
