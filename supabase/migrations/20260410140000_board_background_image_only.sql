-- Doit: убрать пользовательский цвет фона доски; остаётся фон темы приложения или загруженное изображение.

ALTER TABLE public.boards DISABLE TRIGGER boards_enforce_update_permissions;

ALTER TABLE public.boards DROP CONSTRAINT IF EXISTS boards_background_check;
ALTER TABLE public.boards DROP CONSTRAINT IF EXISTS boards_background_type_check;
...
SET
  ...
  background_image_path = NULL,
  background_color = NULL
WHERE background_type = 'image' AND background_image_path IS NULL;

ALTER TABLE public.boards ADD CONSTRAINT boards_background_type_check
  CHECK (background_type IN ('none', 'image'));

ALTER TABLE public.boards ADD CONSTRAINT boards_background_check CHECK (
  (
    background_type = 'none'
  ...
  OR (
    ...
    AND background_image_path IS NOT NULL
    AND background_color IS NULL
  )
);
...
BEGIN
  ...
  VALUES (v_name, v_uid, 'none', NULL, NULL)
...
COMMENT ON FUNCTION public.create_board_with_defaults(text) IS
  'Creates boards row, 4 preset roles with MVP permission matrix, owner membership, 4 default columns (14.4.3), default preview items (14.5.6). Board background defaults to theme (no image).';

ALTER TABLE public.boards ENABLE TRIGGER boards_enforce_update_permissions;
