-- Doit: allow board owner to manage board background files in Storage
  ...
  USING (
    ...
    AND EXISTS (
      ...
      AND (
        ...
        OR public.has_board_permission(b.id, 'board.view')
      )
    )
  );
...
CREATE POLICY board_backgrounds_insert
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      AND (
        b.owner_user_id = auth.uid()
        OR public.has_board_permission(b.id, 'board.change_background')
      )
    )
  );
