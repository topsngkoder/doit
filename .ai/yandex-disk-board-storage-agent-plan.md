...
- При удалении файла из карточки UI должен применять **оптимистичное обновление**: файл исчезает из интерфейса сразу (без ожидания ответа от БД/провайдера и без промежуточного отображения в списке). Если фактическое удаление не завершилось успехом, приложение обязано отправить пользователю уведомление (см. настройку уведомлений **«технические уведомления»**).
- Скачивать файлы может любой пользователь с правом просмотра карточки.
- При скачивании сохраняемое имя файла совпадает с исходным именем из БД (`original_file_name`), как в UI списка вложений, а не с техническим именем на Яндекс.Диске (спец. 11.5).
...
- На **отображении карточки на доске** (карточка в колонке, не модалка) показывается сокращённый список готовых вложений: **не более трёх** однострочных строк (имя файла, без переноса; длинное имя обрезается); **скачивание запускается только по клику на подпись имени файла** (не на всю ширину строки), чтобы остальная зона строки оставалась для открытия карточки и перетаскивания; по смыслу — как кнопка `Скачать` в открытой карточке; вложения сверх лимита видны только в **открытой** карточке. Порядок строк — как в спец. 13.7. Загрузка/удаление с доски по-прежнему не предусмотрены.
...
- Целевой production upload-flow: приложение выполняет авторизацию, валидацию, подготовку `uploading`-записи, получение временного upload URL у Яндекс.Диска и финализацию статуса, но **байты файла идут напрямую из браузера в Яндекс.Диск**, а не через body serverless-функции приложения.
...
- `web/src/app/boards/[boardId]/board-columns-dnd.tsx` (`BoardCardRow`)
  - на тайле карточки в колонке — до трёх однострочных `ready`-вложений полей `Яндекс диск` (спец. 13.7, YDB8.8); порядок как в открытой карточке (`flatten-ready-yandex-attachments-for-board-card-tile.ts`).
...
- Для production на Vercel нельзя опираться на сценарий, где весь multipart-файл сначала принимается route handler'ом приложения: целевой путь для больших файлов — direct upload browser -> Yandex Disk по краткоживущему URL, выданному приложением после server-side проверок.
...
- Для upload использовать двухфазный server-side контракт: `prepare upload` (валидация, путь, `uploading`, временный URL Яндекса) -> direct upload из браузера в Яндекс.Диск -> `complete upload` (подтверждение успеха и перевод в `ready`).
- Клиент не должен получать постоянные токены Яндекса; браузеру разрешено получать только краткоживущий upload URL на один заранее зафиксированный путь файла.
- Для удаления/скачивания использовать server route handlers или server actions; сетевые вызовы к Яндексу и все проверки прав остаются в серверных функциях, недоступных клиенту.
...
5. После этого расширить snapshot и UI уровня `Поля доски`/карточки (включая отображение карточки на доске — `YDB8.8`).
...
| 2026-04-11 | спец. 13.7 + план YDB8.8 | **Отображение карточки на доске** (задание на реализацию): до трёх однострочных имён `ready`-вложений со скачиванием по клику на подпись имени (не на всю ширину строки); спецификация — разд. 3, 6.3, 13.7, критерий 19 п. 15. См. выполнение в строке журнала `YDB8.8` ниже. |
| 2026-04-11 | YDB8.8 | `flatten-ready-yandex-attachments-for-board-card-tile.ts`: плоский порядок — поля `yandex_disk` по `position`, внутри поля как в snapshot. `board-columns-dnd.tsx` → `BoardCardRow`: до 3 строк под превью, `truncate`, ссылки на `cardAttachmentDownloadPath` при `canOpenCardModal` + `integration.status === active`; иначе приглушённый текст; `stopPropagation` на click/pointerdown; в drag-overlay и при `disableOpen` — без кликабельного скачивания. Прокинут `boardId` в `BoardCardRow` / `SortableBoardCard` / `sortableCardProps` / `BoardGridStatic` / overlay. Миграций нет; `npx tsc --noEmit` в `web/` — ок. |
| 2026-04-12 | UX тайла карточки (13.7) | Скачивание с тайла: кликабельна только **подпись имени файла** (`inline-block max-w-full truncate` у ссылки/текста в `BoardCardRow`), не вся строка по ширине карточки — см. спец. 13.7 и критерий 19 п. 15. |
| 2026-04-12 | YDB5.7 / спец. 11.5 | Имя файла при скачивании: GET `.../attachments/[id]/download` больше не делает 302 на Яндекс — сервер получает временный URL, проксирует поток клиенту с `Content-Disposition` по `original_file_name` (`attachment-download-content-disposition.ts`, RFC 5987 `filename*`). `resolveCardAttachmentTemporaryDownloadUrl` выбирает `original_file_name`, `mime_type`; `maxDuration=800` как у upload. Обновлены спец. (11.5, 6.3, 18, 19 п. 16), JSDoc endpoints/контракта. Миграций нет; `npx tsc --noEmit` в `web/` — ок. |
| 2026-04-12 | YDB10.1 | **Аудит матрицы прав интеграции (код + типы).** Сервер: `can_manage_board_yandex_disk_integration` — `boards.owner_user_id = auth.uid()` **или** `is_system_admin` (как YDB1.4 RLS на `board_yandex_disk_integrations`). Все мутации/старт OAuth: `requireBoardYandexDiskIntegrationManagement` → RPC (`oauth/start`, `oauth/callback`, `disconnectBoardYandexDiskIntegrationAction` + RPC `disconnect_board_yandex_disk_integration`). UI кнопок OAuth/«Отключить»: только при `canManageYandexDiskIntegration` из `page.tsx` = `snapshot.is_system_admin` или участник с `is_owner` в snapshot (не зависит от ролевых прав вроде `card_fields.manage`). **Итог по ролям из чеклиста:** владелец доски — да; админ/участник/наблюдатель **без** `is_owner` и **без** sysadmin — нет на сервере и в UI. Полный ручной прогон на стенде — по шагам ниже (разд. 4.1). Миграций нет; `npx tsc --noEmit` в `web/` — ок. |
| 2026-04-12 | YDB10.2 | **Аудит матрицы прав карточки (код + RLS).** **Загрузка:** `validateCardAttachmentUploadRequest` → RPC `can_edit_card_content` + `assertCardYandexDiskFieldDefinition`; `runCardAttachmentUpload` / POST `.../attachments/upload`. RLS `card_attachments` INSERT (YDB4.7): `can_edit_card_content`, `uploaded_by_user_id = auth.uid()`, карточка на доске, `field_definition_id` — определение `yandex_disk` этой доски. **Удаление:** `deleteCardAttachment` — сначала `can_edit_card_content`, затем SELECT/DELETE с `field_definition_id`; RLS DELETE — то же или `cards.delete_any` / `delete_own` по карточке или sysadmin. **Скачивание:** `resolveCardAttachmentTemporaryDownloadUrl` **без** проверки редактирования; строка `ready` только при SELECT под RLS (`status = ready` + `has_board_permission(board_id, 'board.view')` + существование карточки) и совпадении `field_definition_id`; плюс активная интеграция и токен. **Список:** `listReadyCardAttachmentsForViewer` — тот же RLS после `assertCardYandexDiskFieldDefinition`. Подмена `field_definition_id` соседнего поля не отдаёт чужой файл (строка не видна → 404 / пусто). В проекте «просмотр карточки» на уровне БД согласован с `board.view` на доске (как SELECT `cards`). **Итог:** просмотр без редактирования — download/list; редактирование содержимого (по правилам `can_edit_card_content`, включая assignee + `board.view`) — upload/delete; без доступа к доске / без сессии — отказ (RLS/401). Ручной прогон — разд. 4.2. Миграций нет; `npx tsc --noEmit` в `web/` — ок. |
| 2026-04-12 | YDB8.9 | **Оптимистичное удаление + технические уведомления.** Миграция `20260412183000_ydb89_technical_notification_event_type.sql`: `event_type = technical` в CHECK `internal_notifications`, `notification_preferences`, `notification_outbox`; дефолтные строки предпочтений для всех профилей; RPC `enqueue_technical_notification` (SECURITY DEFINER, `auth.uid()`), доставка в `internal_notifications` и `notification_outbox` по каналам с учётом настроек. TS: `technical` в `NOTIFICATION_EVENT_TYPES`, подписи в настройках и email-форматтере. `deleteCardAttachmentAction`: при `ok: false` — вызов RPC с продуктовым заголовком/телом (`yandex-disk-product-messages.ts`). `edit-card-modal.tsx`: `optimisticRemovedYandexAttachmentIds`, строка вложения скрывается до ответа; после ответа — снятие оптимизма, `listReadyCardAttachmentsAction` + `router.refresh()`. `npx tsc --noEmit` в `web/` — ок. **`npx supabase db push` применён на стенде пользователя** (2026-04-12). |

### 4.1) Ручная проверка YDB10.1 (по желанию на своей среде)

1. **Владелец:** открыть доску → «Поля доски» → блок Яндекс.Диск: видны действия подключения/переподключения/отключения (при соответствующих статусах); OAuth уводит на Яндекс и возвращает без `yandex_disk_oauth=forbidden`.
2. **Не владелец с `board.view`** (участник или наблюдатель, в snapshot у пользователя нет `is_owner` для этой доски): тот же блок — только безопасный статус (как в YDB7.3), **без** ссылок OAuth и без «Отключить».
3. **Администратор доски без владения:** если в продукте есть роль с широкими правами, но `boards.owner_user_id` — другой пользователь и у члена `is_owner = false`: убедиться, что OAuth-ссылка из UI недоступна; прямой заход на `/api/yandex-disk/oauth/start?board_id=…` — редирект на доску с `yandex_disk_oauth=forbidden` (или аналог из YDB7.5).
4. **Sysadmin (если используете):** по коду управление разрешено как у владельца — это намеренно (см. миграцию `can_manage_board_yandex_disk_integration`).

### 4.2) Ручная проверка YDB10.2 (по желанию на своей среде)

1. **Только просмотр доски** (роль с `board.view`, без `cards.edit_any` / `cards.edit_own` и не исполнитель карточки): открыть карточку с полем «Яндекс диск» и готовым файлом — виден список, «Скачать» / клик по **имени файла** на тайле работают при `active` интеграции; зона загрузки и «Удалить» недоступны (или ответ с продуктовым текстом «нет права», если вызвать upload/delete вручную).
2. **Редактирование содержимого** (админ с `cards.edit_any`, автор с `cards.edit_own`, или исполнитель с `board.view` по правилам проекта): загрузка и удаление в поле «Яндекс диск» проходят при активной интеграции.
3. **Две области «Яндекс диск» на одной карточке:** убедиться, что скачивание/удаление из поля A не затрагивает файл поля B (другой `field_definition_id` в URL и в action).
4. **Нет доступа к доске:** неавторизованный запрос к GET download — 401; пользователь без членства на доске — вложение не находится (404) из‑за RLS на `card_attachments`.
...
- [x] **YDB4.9 (done)** Добавить server-side завершение и recovery direct upload
  ...
  - **DoD**: direct upload не оставляет недетерминированных `uploading` без контролируемого завершения.
...
- [x] **YDB5.7 (done)** Имя сохраняемого файла при скачивании = `original_file_name` из БД (спец. 11.5)
  - редирект на временный URL Яндекса как единственный ответ клиенту не подходит: браузер берёт имя с Диска (`<attachmentId><extension>`);
  - приложение проксирует поток и выставляет `Content-Disposition` (в т.ч. не-ASCII через `filename*`);
  - **DoD**: имя при сохранении совпадает с отображаемым в карточке; временный URL по-прежнему не кэшируется и не показывается пользователю.
...
- [x] **YDB8.8 (done)** Список вложений на отображении карточки в колонке (спец. 13.7)
  - данные уже в snapshot: `BoardCardListItem.readyAttachmentsByFieldId` — собрать **плоский** список в порядке полей доски и внутри поля в том же порядке, что в открытой карточке;
  - на карточке доски: **не более трёх** строк; каждая строка — **одно** готовое вложение, **одна строка** UI (truncate/ellipsis при длинном имени);
  - **подпись имени файла** в строке — отдельный кликабельный контрол (как дополнительная кнопка на карточке), **не вся ширина строки**: `href` или навигация на `cardAttachmentDownloadPath(boardId, cardId, attachmentId, fieldDefinitionId)` при `canOpenCardModal` (или эквивалент права просмотра) и `integration.status === active` — по аналогии с `YDB8.3`; пустая часть строки — для открытия карточки и drag;
  - при неактивной интеграции — без скачивания (неактивные строки или скрытие списка на тайле, без обхода 13.6);
  - при более чем трёх `ready`-файлах на карточке четвёртое и далее **только** в модалке;
  - **DoD**: критерий приёмки 19 п. 15 спецификации выполнен; `YDB10.4` дополнен сценарием скачивания с тайла карточки.
- [x] **YDB8.9 (done)** Оптимистичное удаление вложения + техническое уведомление при ошибке
  - при нажатии `Удалить` в списке вложений файла соответствующего поля элемент исчезает из UI сразу, не дожидаясь ответа сервера/БД и без показа в списке в промежутке;
  - серверная операция удаления выполняется в фоне; на успех — синхронизация состояния без заметного «мигания»;
  - на ошибку удаления — отправить пользователю уведомление (категория **«технические уведомления»** в настройках уведомлений) и выполнить синхронизацию списка вложений с сервером, чтобы UI вернулся к фактическому состоянию;
  - **DoD**: удаление ощущается мгновенным; при реальном сбое пользователь узнаёт об этом через уведомление и видит актуальный список после синхронизации.
...
- [x] **YDB10.1 (done)** Прогнать матрицу прав интеграции доски
  - владелец доски;
  - администратор без владения;
  - участник;
  - наблюдатель;
  - **DoD**: только владелец может подключать/переподключать/отключать интеграцию. *(Закрыто аудитом кода + RPC/RLS; ручной сценарий — разд. 4.1; плюс sysadmin по политике проекта.)*
- [x] **YDB10.2 (done)** Прогнать матрицу прав карточки
  - просмотр без редактирования;
  - редактирование содержимого;
  - отсутствие доступа к карточке;
  - **DoD**: upload/delete/download соответствуют ролям и не смешиваются. *(Закрыто аудитом серверных модулей + RLS `card_attachments` / `can_edit_card_content`; ручной сценарий — разд. 4.2.)*
...
- [ ] **YDB10.4 (todo)** Прогнать сценарии вложений
  - одиночная загрузка;
  - batch upload;
  - загрузка файла больше лимита body serverless-функции Vercel проходит в production благодаря direct upload, без проксирования полного тела файла через приложение;
  ...
  - **оптимистичное удаление**: файл исчезает из UI сразу; при ошибке удаления приходит уведомление пользователю (если включены **«технические уведомления»**) и список синхронизируется с сервером;
  - удаление карточки с вложениями;
  - скачивание по клику на **подпись имени файла** в строке вложения на **отображении карточки** в колонке (лимит 3 строки, спец. 13.7 / `YDB8.8`);
  - сохранённое на диске имя файла при скачивании совпадает с `original_file_name` (спец. 11.5 / `YDB5.7`);
  - **DoD**: разделы 10, 11 и 12 покрыты без открытых дефектов; сценарий 13.7 проверен.
...
5. После этого расширить данные/UI через `YDB6.*`, `YDB7.*`, `YDB8.*` (включая `YDB8.7` — прогресс и скорость загрузки в карточке; `YDB8.8` — вложения на тайле карточки на доске).
...
- `YDB8.8` зависит от `YDB5.*` (download route), `YDB6.2`/`YDB6.5` (вложения в snapshot) и компонента отображения карточки в колонке; отдельных миграций не требует.
...
- Риск: проксирование скачивания (спец. 11.5) увеличивает трафик и нагрузку на сервер приложения по сравнению с редиректом 302.
  - Контроль: стриминг тела без буферизации целого файла в памяти; согласовать `maxDuration`/лимиты хостинга с максимальным размером вложения (спец. 10.2).
- Риск: upload-flow будет принимать полный multipart через Vercel route handler и упрётся в жёсткий лимит request body раньше продуктового лимита файла.
  - Контроль: целевой production-сценарий — `prepare upload` на сервере и последующий direct upload browser -> Yandex Disk по временному URL.
- Риск: upload-flow может зависнуть в `card_attachments.status = 'uploading'`, если провайдер принял файл асинхронно (`202 Accepted`) или финальный UPDATE статуса сорвался из-за RLS/контекста сессии.
  - Контроль: финальные переходы `uploading -> ready|failed` выполнять через доверенный server-side контур (`service_role`) с явной проверкой обновления строки; для direct upload дополнительно предусмотреть отдельный `complete upload` и служебный reconcile stale-`uploading` по `storage_path` через проверку фактического наличия файла на Яндекс.Диске.
...
- загрузка, скачивание и удаление работают только через приложение и только по правам карточки; при скачивании имя сохраняемого файла совпадает с `original_file_name` (спец. 11.5);
...
- в карточке появились файловые поля `Яндекс диск` с корректными состояниями и разделением по полям, а при загрузке — прогресс и скорость по спец. 13.5; на отображении карточки на доске — до трёх однострочных вложений со скачиванием по **клику на подпись имени файла** по спец. 13.7;
...
| Дата | Что сделано |
|------|-------------|
| 2026-04-12 | **Архитектурная коррекция upload-flow для Vercel:** в спецификации и плане зафиксирован целевой production-сценарий `prepare upload` -> direct upload browser -> Yandex Disk -> `complete upload`, чтобы лимит request body serverless-функций не ограничивал продуктовый лимит 1 ГБ на файл. |
...
| 2026-04-11 | **Вложения на тайле карточки (13.7):** спецификация — разд. 3, 6.3, 13.7, критерий 19 п. 15; план — разд. 0, 1.2, 3–6, 8, `YDB8.8`, `YDB10.4`. |
| 2026-04-12 | **Имя файла при скачивании (спец. 11.5):** в спецификации — новый п. 11.5, уточнены 6.3 и 18, критерий 19 п. 16; в плане — разд. 0, 7, 8, `YDB5.7`, журнал прогресса, `YDB10.4`. Реализация — прокси GET download + `Content-Disposition`. |
| 2026-04-12 | **Тайл карточки — зона клика вложения:** спецификация — п. 13.7, критерий 19 п. 15; план — разд. 0, `YDB8.8`, `YDB10.4` п. 1, разд. 8, журнал прогресса. Уточнено: скачивание только по подписи имени файла, не на всю ширину строки. |
| 2026-04-12 | **Оптимистичное удаление + технические уведомления:** в требования плана и чеклист добавлено правило: при удалении файла из карточки он исчезает из UI сразу, не дожидаясь ответа сервера/БД; при фактическом сбое — уведомление пользователю (настройка **«технические уведомления»**) и последующая синхронизация списка с сервером. |
