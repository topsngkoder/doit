...
| 2026-04-11 | спец. 13.7 + план YDB8.8 | **Отображение карточки на доске** (задание на реализацию): до трёх однострочных кликабельных имён `ready`-вложений; спецификация — разд. 3, 6.3, 13.7, критерий 19 п. 15. См. выполнение в строке журнала `YDB8.8` ниже. |
...
- [x] **YDB8.8 (done)** Список вложений на отображении карточки в колонке (спец. 13.7)
  ...
  - строка — отдельный кликабельный контрол (как дополнительная кнопка на карточке): `href` или навигация на `cardAttachmentDownloadPath(boardId, cardId, attachmentId, fieldDefinitionId)` при `canOpenCardModal` (или эквивалент права просмотра) и `integration.status === active` — по аналогии с `YDB8.3`;
...
- [ ] **YDB10.4 (todo)** Прогнать сценарии вложений
  ...
  - скачивание по клику на строку вложения на **отображении карточки** в колонке (лимит 3 строки, спец. 13.7 / `YDB8.8`);
...
- в карточке появились файловые поля `Яндекс диск` с корректными состояниями и разделением по полям, а при загрузке — прогресс и скорость по спец. 13.5; на отображении карточки на доске — до трёх однострочных кликабельных вложений со скачиванием по спец. 13.7;
