...
## Прогресс (журнал)
- **2026-04-10** — **T24 DONE**: выполнена диагностика по изменённой теме через IDE diagnostics (`ReadLints`) — новых линтерных ошибок не обнаружено. Попытка `npm run lint` в `web/` прервана интерактивным мастером первичной настройки ESLint (`next lint`), поэтому в рамках текущего репозитория источником проверки принята диагностика IDE. Миграции БД не требовались.
- **2026-04-10** — **T23 DONE**: ручная регрессия `dark/light` по ключевым экранам подтверждена пользователем («все ок»). Критерий визуальных регрессий по основным сценариям считается выполненным.
- **2026-04-10** — **T23 IN PROGRESS (нужно участие пользователя)**: проверил возможность автопрогона регрессии — в проекте нет e2e-раннера (`playwright`/`cypress`), поэтому визуальные сценарии `dark/light` по ключевым экранам требуют ручной проверки в браузере. Подготовлен ручной чек-лист для закрытия T23 (ниже). После твоего подтверждения по чек-листу переведу T23 в `DONE` и перейду к `T24`.
...
- **2026-04-10** — **T21 DONE**: выровнены состояния focus/hover в board settings и участниках доски (§14): `board-members.tsx` — убраны `focus-visible:ring-sky-500` и `ring-offset-slate-950`, интерактивы переведены на `focus-ring-app`, hover-состояния на `text-app-*`/`bg-app-surface-*`, также приведены status-тексты/бейджи модалки к семантическим токенам; `board-settings-menu.tsx` — триггер и flyout на `popup-panel`, `border-app-*`, `hover:bg-app-surface-muted`; `invite-member-button.tsx` — trigger класса приглашения и сообщения формы на токенах. Проверка линтера по изменённым файлам: ошибок нет. Миграции БД не требовались.
- **2026-04-10** — **T22 DONE**: финальная чистка hardcoded dark values в затронутой board UI-зоне (§15): `board-labels-button.tsx`, `board-fields-button.tsx`, `board-card-preview-button.tsx` переведены с `slate/sky/rose` utility-классов на семантические токены (`text-app-*`, `border-app-*`, `bg-app-*`, `surface-muted`, `checkbox-app`, `text-app-validation-error`, destructive hover через `--danger-subtle-*`). Color chips и пользовательские цвета сохранены без изменений. Остаточные совпадения поиска в `board-settings-menu.tsx` / `edit-card-modal.tsx` — ложноположительные по строковым литералам, не hardcoded-тёмные классы. Проверка линтера по изменённым файлам: ошибок нет. Миграции БД не требовались.
...
## Результат T17 — вуаль board image

| Что | Где |
| --- | --- |
| Токен вуали | `globals.css` — `--board-image-veil` в `:root` и `html[data-theme="light"]` |
| Светлая тема | `--board-image-veil: rgba(255, 255, 255, 0.18)` |
| Тёмная тема | `--board-image-veil: rgba(2, 6, 23, 0.25)` (сохранение текущей модели) |
| Применение к фону доски | `board-background-frame.tsx` — `linear-gradient(var(--board-image-veil), var(--board-image-veil)), url(...)` |
| Инварианты | Signed URL / кеш / выбор пользовательского фона не изменены |

## Результат T18 — канва, колонки, карточки

| Что | Где |
| --- | --- |
| Подложка колонок §11 | `globals.css` — `--board-column-bg`; `board-columns-dnd.tsx` — колонки на `bg: var(--board-column-bg)`, `border-app-default`, `shadow-[var(--shadow-card)]` |
| Карточки §11 | `board-columns-dnd.tsx` — карточки на `bg-app-surface`, границы `border-app-default` + hover `border-app-strong`, фокус `--focus-ring` |
| Пустые состояния и drop-zone | `board-columns-dnd.tsx` — dashed/empty блоки на `border-app-divider` / `border-app-strong`, без `slate-*` |
...
## Результат T21 — focus/hover/disabled состояния

| Что | Где |
| --- | --- |
| Focus в участниках доски | `board-members.tsx` — интерактивы переведены на `focus-ring-app` вместо `ring-offset-slate-950`/`ring-sky-500` |
| Hover- и chip-состояния в участниках | `board-members.tsx` — кнопки/ссылки/аватары на `bg-app-surface-*`, `text-app-*`, `ring-[color:var(--bg-page)]` |
| Меню настроек доски | `board-settings-menu.tsx` — trigger и popup на `border-app-*`, `popup-panel`, hover пунктов `bg-app-surface-muted` |
| Кнопка приглашения | `invite-member-button.tsx` — default trigger + тексты статуса формы на семантических токенах |
| Disabled-правило | По затронутым интерактивам сохраняется `disabled:opacity-60` (базовые примитивы `Button`/`Input`, профильный avatar action) |
...
| T21 | DONE | Выровнять focus/hover/disabled states по всей затронутой UI-зоне | все затронутые UI и screen файлы | T06-T20 | Focus явно виден, disabled читаем, hover мягкий и заметный |
| T22 | DONE | Финально вычистить hardcoded dark values из затронутых файлов | все изменённые файлы | T05-T21 | В затронутых местах не осталось жёстких тёмных поверхностей вне исключений |
| T23 | DONE | Провести ручную регрессионную проверку `dark` и `light` на ключевых экранах | приложение целиком | T11-T22 | Обе темы работают без визуальных регрессий |
| T24 | DONE | Прогнать диагностику по изменённым файлам и исправить новые ошибки | изменённые файлы | T23 | Нет новых линтерных ошибок, связанных с темой |

## Чек-лист ручной регрессии для закрытия T23
1. Запусти `web` в dev-режиме и открой приложение в браузере.
2. Сбрось `localStorage` ключа `doit:theme` и проверь старт в `dark` без вспышки темы.
3. На `Личный кабинет` переключи тему на `light`; обнови страницу и открой новую вкладку — тема должна сохраниться.
4. Проверь экраны: лендинг, вход, регистрация, профиль, доски, уведомления, настройки уведомлений — без тёмных hardcoded-плашек в `light`.
5. Открой доску с цветным/изображением фоном: читаемость колонок/карточек, светлая вуаль на image-фоне, корректные chips меток/select-опций.
6. Открой card modal и правую колонку комментариев: светлые поверхности, разделители, заметные focus/hover, disabled = 60% opacity.
7. Переключи обратно в `dark` и повторно просмотри ключевые экраны на отсутствие регрессий относительно прежнего поведения.
